...
RF24AudioJ rfAudioJ(radio,1);     // Set up the audio using the radio, and set to radio number 1
void setup() {                  
...
void loop() {
    //if(Serial.available()){                                           // Receive and analyze incoming serial data
          ...
          // rfAudio.transmit();Serial.println("Transmitiendo");                     // Send an r or an s over serial to control playback
          //rfAudio.receive(); Serial.println("Recibiendo"); 
