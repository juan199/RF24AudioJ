...
RF24AudioJ rfAudioJ(radio,1);     // Set up the audio using the radio, and set to radio number 0
void setup() {                  
  ...
  rfAudioJ.begin();              // Start up the radio and audio libraries
  //rfAudioJ.setVolume(4);
}
void loop() {
    //if(Serial.available()){                                           // Receive and analyze incoming serial data
       
           rfAudioJ.transmit();Serial.println("Transmitiendo");                     // Send an r or an s over serial to control playback
            //case 's': rfAudio.receive(); Serial.println("Recibiendo"); break;
