...
/*void RF24AudioJ::volume(bool upDn){
	vol(upDn);
}

void RF24AudioJ::setVolume(char vol) {
    volMod = vol - 4 ;
}*/
...
void handleButtons(){
  ...
  if(!digitalRead(VOL_UP_PIN)){                 //If the volume pin is high, raise the volume
    if(millis()-volTime > 200){                //De-bounce of buttons by 200ms
        //vol(1); volTime = millis();			// ##### Juan: Deshabilitar subida y bajada de volumen... #####
     }
  }/*else
...
void RF24AudioJ::broadcast(byte radioID){
  ...
  ISR(TIMER1_COMPA_vect){ 
	 ...
	 if(buffEmpty[!whichBuff] == 0){                      			 // If a buffer is ready to be sent
			...
			radi.writeFast(&buffer[a],32);						 // TEO APAGÓ ESTO!!!! OJO!!!! (ir a RF24.cpp función: startFastWrite )
...
void TX(){
		//TIMSK1 &= ~(_BV(OCIE1A));          				//Disable Radio Tx
